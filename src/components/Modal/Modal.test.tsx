...
describe('Modal', () => {
  ...
  it('Should render title when provided', () => {
    render(
      <Modal title="Modal title" positiveAction={positiveAction}>
        Modal content
      </Modal>
    );
  ...
  it('Should not render close when hasClose is false', () => {
    render(
      <Modal title="Modal title" positiveAction={positiveAction} hasClose={false}>
        Modal content
      </Modal>
    );
  ...
  it('Should render close when hasClose is true (and by default), and add aria-label from closeAriaLabel', () => {
    render(
      <Modal title="Modal title" positiveAction={positiveAction} closeAriaLabel="close modal">
        Modal content
      </Modal>
    );
  ...
  it('Should render negative action as subtle button style when close is present', () => {
    render(
      <Modal title="Modal title" positiveAction={positiveAction} negativeAction={negativeAction}>
        Modal content
      </Modal>
    );
  ...
  it('Should render negative action as default button style when close is not present', () => {
    render(
      <Modal title="Modal title" hasClose={false} positiveAction={positiveAction} negativeAction={negativeAction}>
        Modal content
      </Modal>
    );
    expect(screen.getByText('Negative action')).toBeInTheDocument();
